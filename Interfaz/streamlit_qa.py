...
API_HOST =  os.environ.get('qa_ip', 'localhost')
API_PORT =  os.environ.get('qa_port', 85)
NOR_RESP =  os.environ.get('norm_resp', 1)
NOR_DOC =  os.environ.get('norm_doc', 5)
AV_RESP =  os.environ.get('av_resp', 1)
AV_DOC =  os.environ.get('av_doc', 10)
direccion = str(API_HOST)+':'+str(API_PORT)
@st.cache
def classify(num):
    if num == 0:
        return 'setosa'
...
def main():
    ...
    print("conexion: ", requests.get('http://'+direccion+'/docs'))
    ...
    question_es=st.text_input("Ingrese su pregunta", value="what are the symptoms of covid-19?", help="Recuerde formular de forma correcta su pregunta o podria obtener respuestas incorrectas")
    ...
    if st.button('Aceptar'):
        ...
        with st.spinner('\n Obteniendo Respuesta...'):
            ...
            if(not len(q)>1):
                ...
                if busqueda == 'Normal':
                    top_k_reader = NOR_RESP
                    top_k_retriever = NOR_DOC
                else:
                    top_k_reader = AV_RESP
                    top_k_retriever = AV_DOC
                #print(idioma)
                print(busqueda)
                print(question)
                headers = {
                ...
                try:
                    ...
                    response = requests.post(f'http://'+direccion+'/query', headers=headers, data=json.dumps(data))
